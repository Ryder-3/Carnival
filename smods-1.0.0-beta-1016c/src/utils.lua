...
function SMODS._save_d_u(o)
    assert(not o._discovered_unlocked_overwritten,
        ("Internal: discovery/unlocked of object \"%s\" should not be overwritten at this stage."):format(o and o.key or
        "UNKNOWN"))
...
function SMODS.process_loc_text(ref_table, ref_value, loc_txt, key)
    local target = (type(loc_txt) == 'table') and
        ((G.SETTINGS.real_language and loc_txt[G.SETTINGS.real_language]) or loc_txt[G.SETTINGS.language] or loc_txt['default'] or loc_txt['en-us']) or
        loc_txt
...
local function parse_loc_file(file_name, force, mod_id)
    ...
    if file_name:lower():match("%.json$") then
        ...
        loc_table = assert(loadstring(NFS.read(file_name),
            ('=[SMODS %s "%s"]'):format(mod_id, string.match(file_name, '[^/]+/[^/]+$'))))()
    end
...
function SMODS.eval_this(_card, effects)
    sendWarnMessage(
    'SMODS.eval_this is deprecated. All calculation stages now support returning effects directly. Effects evaluated using this function are out of order and may not use the correct sound pitch.',
        'Util')
    if effects then
...
function SMODS.create_card(t)
    ...
    if not t.key and t.set == 'Playing Card' or t.set == 'Base' or t.set == 'Enhanced' or (not t.set and (t.front or t.rank or t.suit)) then
        t.set = t.set == 'Playing Card' and
        (t.enhancement and 'Base' or (pseudorandom('front' .. (t.key_append or '') .. G.GAME.round_resets.ante) > (t.enhanced_poll or 0.6) and 'Enhanced' or 'Base')) or
        t.set or 'Base'
        ...
        if not t.front and (t.suit or t.rank) then
            t.suit = t.suit and (SMODS.Suits["" .. t.suit] or {}).card_key or t.suit or
                pseudorandom_element(SMODS.Suits, pseudoseed('front' .. (t.key_append or '') .. G.GAME.round_resets.ante))
                .card_key
            t.rank = t.rank and (SMODS.Ranks["" .. t.rank] or {}).card_key or t.rank or
                pseudorandom_element(SMODS.Ranks, pseudoseed('front' .. (t.key_append or '') .. G.GAME.round_resets.ante))
                .card_key
        end
...
function SMODS.debuff_card(card, debuff, source)
    ...
    if debuff == 'reset' then
        card.ability.debuff_sources = {}; return
    end
        ...
        for i, mod in ipairs(mods) do
            ...
            badges[#badges + 1] = {
                ...
                nodes = {
                    {
                        n = G.UIT.R,
                        config = { align = "cm", colour = mod.badge_colour or G.C.GREEN, r = 0.1, minw = 2, minh = 0.36, emboss = 0.05, padding = 0.03 * size },
                        nodes = {
...
V_MT = {
    ...
    __call = function(_, str)
        str = str or '0.0.0'
        local _, _, major, minorFull, minor, patchFull, patch, rev = string.find(str,
            '^(%d+)(%.?([%d%*]*))(%.?([%d%*]*))(.*)$')
...
G.FUNCS.update_collab_cards = function(key, suit, silent)
    ...
    local palette = deckskin.palette_map and deckskin.palette_map[G.SETTINGS.colour_palettes[suit] or ''] or
    (deckskin.palettes or {})[1]
    ...
    if diff_order then
        ...
        for i, r in ipairs(d_ranks) do
            ...
            local card = Card(G.cdds_cards.T.x + G.cdds_cards.T.w / 2, G.cdds_cards.T.y + G.cdds_cards.T.h / 2,
                G.CARD_W * 1.2, G.CARD_H * 1.2, G.P_CARDS[card_code], G.P_CENTERS.c_base)
...
G.FUNCS.update_suit_colours = function(suit, skin, palette_num)
    ...
    if G.SETTINGS.colour_palettes[suit] == 'lc' or G.SETTINGS.colour_palettes[suit] == 'hc' then
        new_colour_proto = G.C
        ["SO_" .. ((G.SETTINGS.colour_palettes[suit] == 'hc' and 2) or (G.SETTINGS.colour_palettes[suit] == 'lc' and 1))]
        [suit]
    end
    if skin and not skin.outdated then
        local palette = (palette_num and skin.palettes[palette_num]) or
        skin.palette_map and skin.palette_map[G.SETTINGS.colour_palettes[suit] or '']
...
SMODS.smart_level_up_hand = function(card, hand, instant, amount)
    ...
    if not (instant or SMODS.displayed_hand == hand) then
        update_hand_text({ sound = 'button', volume = 0.7, pitch = 0.8, delay = 0.3 },
            { handname = localize(hand, 'poker_hands'), chips = G.GAME.hands[hand].chips, mult = G.GAME.hands[hand].mult, level =
...
SMODS.calculate_individual_effect = function(effect, scored_card, key, amount, from_edition)
    if SMODS.Scoring_Parameter_Calculation[key] then
        return SMODS.Scoring_Parameters[SMODS.Scoring_Parameter_Calculation[key]]:calc_effect(effect, scored_card, key,
            amount, from_edition)
    end
    ...
    if (key == 'p_dollars' or key == 'dollars' or key == 'h_dollars') and amount then
        ...
        if not effect.remove_default_message then
            if effect.dollar_message then
                ...
                effect.message_card or effect.juice_card or scored_card or effect.card or effect.focus, 'extra', nil,
                    percent, nil, effect.dollar_message)
            else
                card_eval_status_text(
                effect.message_card or effect.juice_card or scored_card or effect.card or effect.focus, 'dollars', amount,
                    percent)
            end
        end
        SMODS.calculate_context({
            money_altered = true,
            amount = amount,
            from_shop = (G.STATE == G.STATES.SHOP or G.STATE == G.STATES.SMODS_BOOSTER_OPENED or G.STATE == G.STATES.SMODS_REDEEM_VOUCHER) or
            nil,
                    ...
                    for i = curr_size + 1, new_size do
                        if not first then
                            ...
                            if not context.post_trigger and SMODS.optional_features.post_trigger then
                                SMODS.calculate_context(
                                { blueprint_card = context.blueprint_card, post_trigger = true, other_card = _card, other_context =
                                context, other_ret = eval }, post)
                            end
                        end
                        first = nil
                        if next(post) then
...
SMODS.calculate_retriggers = function(card, context, _ret)
    ...
    for _, area in ipairs(SMODS.get_card_areas('jokers')) do
        for _, _card in ipairs(area.cards) do
            local eval, post = eval_card(_card,
    ...
    for _, area in ipairs(SMODS.get_card_areas('individual')) do
        local eval, post = SMODS.eval_individual(area,
            { retrigger_joker_check = true, other_card = card, other_context = context, other_ret = _ret })
        if next(eval) then
            if next(post) then SMODS.trigger_effects({ post }, _card) end
            for key, value in pairs(eval) do
    ...
    if _type == 'individual' then
        for _, area in ipairs(SMODS.get_card_areas('individual')) do
            ...
            if args and args.main_scoring and eval.individual then
                ...
                eval.individual.message_card = eval.individual.message_card or eval.individual.card or context
                .other_card
            end
            local effects = { eval }
            for _, v in ipairs(post) do effects[#effects + 1] = v end
...
function SMODS.push_to_context_stack(context, func)
    if not context or type(context) ~= "table" then
        sendWarnMessage(
        ('Called SMODS.push_to_context_stack with invalid context \'%s\', in function \'%s\''):format(context, func),
            'Util')
    end
...
function SMODS.pop_from_context_stack(context, func)
    ...
    if len <= 0 or SMODS.context_stack[len].context ~= context then
        sendWarnMessage(
        ('Called SMODS.pop_from_context_stack with invalid context \'%s\', in function \'%s\''):format(context, func),
            'Util')
    else
        SMODS.context_stack[len].count = SMODS.context_stack[len].count - 1
...
function SMODS.calculate_context(context, return_table, no_resolve)
    ...
    flags[#flags + 1] = SMODS.calculate_card_areas('jokers', context, return_table,
        { joker_area = true, has_area = has_area })
...
function SMODS.get_card_areas(_type, _context)
    ...
    if _type == 'individual' then
        ...
        if G.GAME.challenge then t[#t + 1] = { object = SMODS.Challenges[G.GAME.challenge], scored_card = G.deck.cards
            [1] or G.deck } end
...
function SMODS.eval_individual(individual, context)
    ...
    if (eff and not eff.no_retrigger) or triggered then
        ...
        if not context.post_trigger and not context.retrigger_joker_check and SMODS.optional_features.post_trigger then
            SMODS.calculate_context(
            { blueprint_card = context.blueprint_card, post_trigger = true, other_card = individual.object, other_context =
            context, other_ret = ret }, post_trig)
        end
    end
...
SMODS.get_optional_features = function()
    for _, mod in ipairs(SMODS.mod_list) do
        if mod.can_load and mod.optional_features then
            local opt_features = type(mod.optional_features) == 'function' and mod.optional_features() or
...
function SMODS.add_voucher_to_shop(key, dont_save)
    ...
    local card = Card(G.shop_vouchers.T.x + G.shop_vouchers.T.w / 2,
        G.shop_vouchers.T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, G.P_CENTERS[key],
...
function SMODS.add_booster_to_shop(key)
    ...
    local card = Card(G.shop_booster.T.x + G.shop_booster.T.w / 2,
        G.shop_booster.T.y, G.CARD_W * 1.27, G.CARD_H * 1.27, G.P_CARDS.empty, G.P_CENTERS[key],
        { bypass_discovery_center = true, bypass_discovery_ui = true })
    ...
    for _, part in ipairs(lines) do
            ...
            final_line[#final_line].nodes[1] = {
                ...
                config = {
                    ...
                    object = DynaText({
                        ...
                        spacing = _spacing,
                        font = SMODS.Fonts[part.control.f] or
                        (tonumber(part.control.f) and G.FONTS[tonumber(part.control.f)]),
        ...
        elseif part.control.X or part.control.B then
            final_line[#final_line + 1] = {
                n = G.UIT.C,
                config = { align = "m", colour = part.control.B and args.vars.colours[tonumber(part.control.B)] or loc_colour(part.control.X), r = 0.05, padding = 0.03, res = 0.15 },
                nodes = {
                    {
                        n = G.UIT.T,
                        config = {
                            ...
                            loc_colour(part.control.C or nil),
                            font = SMODS.Fonts[part.control.f] or
                            (tonumber(part.control.f) and G.FONTS[tonumber(part.control.f)]),
                            underline = part.control.u and loc_colour(part.control.u),
                            scale = 0.32 * (part.control.s and tonumber(part.control.s) or args.scale or 1) * desc_scale
            ...
            final_line[#final_line + 1] = {
                n = G.UIT.T,
                config = {
                    detailed_tooltip = part.control.T and (G.P_CENTERS[part.control.T] or G.P_TAGS[part.control.T]) or
                    nil,
                    text = assembled_string,
                    shadow = args.shadow,
                    colour = part.control.V and args.vars.colours[tonumber(part.control.V)] or
                    not part.control.C and args.text_colour or loc_colour(part.control.C or nil, args.default_col),
                    font = SMODS.Fonts[part.control.f] or
                    (tonumber(part.control.f) and G.FONTS[tonumber(part.control.f)]),
                    underline = part.control.u and loc_colour(part.control.u),
                    scale = 0.32 * (part.control.s and tonumber(part.control.s) or args.scale or 1) * desc_scale
...
function SMODS.info_queue_desc_from_rows(desc_nodes, empty, maxw)
    ...
    return {
        n = G.UIT.R,
        config = { align = "cm", colour = desc_nodes.background_colour or empty and G.C.CLEAR or G.C.UI.BACKGROUND_WHITE, r = 0.1, emboss = not empty and 0.05 or nil, filler = true, main_box_flag = desc_nodes.main_box_flag and true or nil },
        nodes = {
...
function SMODS.destroy_cards(cards, bypass_eternal, immediate, skip_anim)
    ...
    if next(playing_cards) then SMODS.calculate_context({ scoring_hand = cards, remove_playing_cards = true, removed =
        playing_cards }) end
...
function SMODS.update_hand_limit_text(play, discard)
    if play then SMODS.hand_limit_strings.play = G.GAME.starting_params.play_limit ~= 5 and
        localize('b_limit') .. math.max(1, G.GAME.starting_params.play_limit) or '' end
    if discard then SMODS.hand_limit_strings.discard = G.GAME.starting_params.discard_limit ~= 5 and
        localize('b_limit') .. math.max(0, G.GAME.starting_params.discard_limit) or '' end
end
...
function SMODS.merge_effects(...)
    ...
    for _, eff in ipairs(t) do
        ...
        while current.extra ~= nil do
            ...
            assert(type(current.extra) == 'table',
                ("\"%s\" is not a valid calculate return."):format(tostring(current.extra)))
...
function SMODS.get_probability_vars(trigger_obj, base_numerator, base_denominator, identifier, from_roll, no_mod)
    ...
    local additive = SMODS.calculate_context(
    { mod_probability = true, from_roll = from_roll, trigger_obj = trigger_obj, identifier = identifier, numerator =
    base_numerator, denominator = base_denominator }, nil, not from_roll)
    additive.numerator = (additive.numerator or base_numerator) *
    ((G.GAME and G.GAME.probabilities.normal or 1) / (2 ^ #SMODS.find_card('j_oops')))
    local fixed = SMODS.calculate_context(
    { fix_probability = true, from_roll = from_roll, trigger_obj = trigger_obj, identifier = identifier, numerator =
    additive.numerator or base_numerator, denominator = additive.denominator or base_denominator }, nil, not from_roll)
    return fixed.numerator or additive.numerator or base_numerator,
        fixed.denominator or additive.denominator or base_denominator
end
...
function SMODS.pseudorandom_probability(trigger_obj, seed, base_numerator, base_denominator, identifier, no_mod)
    ...
    SMODS.post_prob[#SMODS.post_prob + 1] = { pseudorandom_result = true, result = result, trigger_obj = trigger_obj, numerator =
    numerator, denominator = denominator, identifier = identifier or seed }
    return result
end
...
function SMODS.is_eternal(card, trigger)
    ...
    SMODS.calculate_context({ check_eternal = true, other_card = card, trigger = trigger, no_blueprint = true, },
        calc_return)
...
G.FUNCS.SMODS_scoring_calculation_function = function(e)
    ...
    if G.GAME.current_scoring_calculation.update_ui then
        ...
        if G.GAME.current_scoring_calculation.text and operator then
            operator.children[1].config.text = type(G.GAME.current_scoring_calculation.text) == 'function' and
            G.GAME.current_scoring_calculation:text() or G.GAME.current_scoring_calculation.text
        end
        if G.GAME.current_scoring_calculation.colour and operator then
            operator.children[1].config.colour = type(G.GAME.current_scoring_calculation.colour) == 'function' and
            G.GAME.current_scoring_calculation:colour() or G.GAME.current_scoring_calculation.colour
        end
        if operator and (type(G.GAME.current_scoring_calculation.colour) == 'function' or type(G.GAME.current_scoring_calculation.text) == 'function') then
            operator.UIBox:recalculate() end
    end
end

SMODS.calculate_round_score = function(flames)
    if not G.GAME.current_scoring_calculation then return 0 end
    return G.GAME.current_scoring_calculation:func(SMODS.get_scoring_parameter('chips', flames),
        SMODS.get_scoring_parameter('mult', flames), flames)
end
...
function SMODS.scale_card(card, args)
    ...
    for _, area in ipairs(SMODS.get_card_areas('jokers')) do
        for _, _card in ipairs(area.cards) do
            ...
            if obj.calc_scaling and type(obj.calc_scaling) == "function" then
                ...
                if ret then
                    if ret.override_value and not args.block_overrides.value then
                        initial = ret.override_value.value; SMODS.calculate_effect(ret.override_value, _card)
                    end
                    if ret.override_scalar_value and not args.block_overrides.scalar then
                        scalar_value = ret.override_scalar_value.value; SMODS.calculate_effect(ret.override_scalar_value,
                            _card)
                    end
                    if ret.override_message and not args.block_overrides.message then scaling_message = SMODS
                        .merge_defaults(ret.override_message, scaling_message) end
                    if ret.post then
                        ret.post.source = _card; scaling_responses[#scaling_responses + 1] = ret.post
                    end
    ...
    scaling_message = scaling_message or {
        message = localize(args.message_key and
        { type = 'variable', key = args.message_key, vars = { args.message_key == 'a_xmult' and args.ref_table[args.ref_value] or scalar_value } } or
        'k_upgrade_ex'),
...
function SMODS.quip(quip_type)
    ...
    for k, v in pairs(SMODS.JimboQuips) do
        ...
        local deck = G.P_CENTERS[G.GAME.selected_back.effect.center.key] or
        SMODS.Centers[G.GAME.selected_back.effect.center.key]
        ...
        if v.filter and type(v.filter) == 'function' then
            add = in_pool and
            (add or (mod_pool_opts and mod_pool_opts.override_base_checks) or (deck_pool_opts and deck_pool_opts.override_base_checks) or (pool_opts and pool_opts.override_base_checks))
        end
        if v.type and v.type ~= quip_type then
            add = false
        end
        if add then
            local weight = (mod_pool_opts and mod_pool_opts.weight and math.max(1, math.floor(mod_pool_opts.weight))) or
            (deck_pool_opts and deck_pool_opts.weight and math.max(1, math.floor(deck_pool_opts.weight))) or
            (pool_opts and pool_opts.weight and math.max(1, math.floor(pool_opts.weight))) or 1
...
function SMODS.challenge_is_unlocked(challenge, k)
    ...
    elseif type(challenge.unlocked) == 'boolean' then
        ...
        challenge_unlocked = G.PROFILES[G.SETTINGS.profile].challenges_unlocked and
        (G.PROFILES[G.SETTINGS.profile].challenges_unlocked >= (k or 0))
    end
    ...
    if SMODS.should_handle_limit(self) then
        ...
        if G.hand and self == G.hand and card_limit - card_slots > 0 then
            ...
            elseif G.STATE == G.STATES.SELECTING_HAND then
                G.FUNCS.draw_from_deck_to_hand(math.min(card_limit - card_slots,
                    (self.config.card_limit + card_limit - card_slots) - #self.cards))
            end
            G.E_MANAGER:add_event(Event({
                trigger = 'immediate',
                func = function()
